...
#define LOCTEXT_NAMESPACE "UNiagaraGPURibbonRendererProperties"

TArray<TWeakObjectPtr<UNiagaraGPURibbonRendererProperties>> UNiagaraGPURibbonRendererProperties::RibbonRendererPropertiesToDeferredInit;


FNiagaraGPURibbonShapeCustomVertex::FNiagaraGPURibbonShapeCustomVertex()
	: Position(0.0f)
	, Normal(0.0f)
	, TextureV(0.0f)
{
...
FNiagaraGPURibbonUVSettings::FNiagaraGPURibbonUVSettings()
	: DistributionMode(ENiagaraGPURibbonUVDistributionMode::ScaledUsingRibbonSegmentLength)
	, LeadingEdgeMode(ENiagaraGPURibbonUVEdgeMode::Locked)
	, TrailingEdgeMode(ENiagaraGPURibbonUVEdgeMode::Locked)
	, TilingLength(100.0f)
...
UNiagaraGPURibbonRendererProperties::UNiagaraGPURibbonRendererProperties()
	: Material(nullptr)
	...
	, UV0AgeOffsetMode_DEPRECATED(ENiagaraGPURibbonAgeOffsetMode::Scale)
	...
	, TubeSubdivisions(3)
	, CurveTension(0.f)
	, TessellationMode(ENiagaraGPURibbonTessellationMode::Automatic)
...
FNiagaraRenderer* UNiagaraGPURibbonRendererProperties::CreateEmitterRenderer(ERHIFeatureLevel::Type FeatureLevel, const FNiagaraEmitterInstance* Emitter, const UNiagaraComponent* InComponent)
{
	FNiagaraRenderer* NewRenderer = new FNiagaraRendererGPURibbons(FeatureLevel, this, Emitter);
	NewRenderer->Initialize(this, Emitter, InComponent);
	return NewRenderer;
}

#if WITH_EDITORONLY_DATA
void UpgradeUVSettings(FNiagaraGPURibbonUVSettings& UVSettings, float TilingDistance, const FVector2D& Offset, const FVector2D& Scale)
{
	if (TilingDistance == 0)
	{
		UVSettings.LeadingEdgeMode = ENiagaraGPURibbonUVEdgeMode::SmoothTransition;
		UVSettings.TrailingEdgeMode = ENiagaraGPURibbonUVEdgeMode::SmoothTransition;
		UVSettings.DistributionMode = ENiagaraGPURibbonUVDistributionMode::ScaledUniformly;
	}
	else
	{
		UVSettings.LeadingEdgeMode = ENiagaraGPURibbonUVEdgeMode::Locked;
		UVSettings.TrailingEdgeMode = ENiagaraGPURibbonUVEdgeMode::Locked;
		UVSettings.DistributionMode = ENiagaraGPURibbonUVDistributionMode::TiledOverRibbonLength;
...
void UNiagaraGPURibbonRendererProperties::PostLoad()
{
	Super::PostLoad();
...
FNiagaraBoundsCalculator* UNiagaraGPURibbonRendererProperties::CreateBoundsCalculator()
{
	return new FNiagaraBoundsCalculatorHelper<false, false, true>();
}

void UNiagaraGPURibbonRendererProperties::GetUsedMaterials(const FNiagaraEmitterInstance* InEmitter, TArray<UMaterialInterface*>& OutMaterials) const
{
...
bool UNiagaraGPURibbonRendererProperties::PopulateRequiredBindings(FNiagaraParameterStore& InParameterStore)
{
...
void UNiagaraGPURibbonRendererProperties::UpdateSourceModeDerivates(ENiagaraRendererSourceDataMode InSourceMode, bool bFromPropertyEdit)
{
...
void UNiagaraGPURibbonRendererProperties::PostInitProperties()
{
...
void UNiagaraGPURibbonRendererProperties::InitCDOPropertiesAfterModuleStartup()
{
	UNiagaraGPURibbonRendererProperties* CDO = CastChecked<UNiagaraGPURibbonRendererProperties>(UNiagaraGPURibbonRendererProperties::StaticClass()->GetDefaultObject());
	CDO->InitBindings();

	for (TWeakObjectPtr<UNiagaraGPURibbonRendererProperties>& WeakRibbonRendererProperties : RibbonRendererPropertiesToDeferredInit)
	{
...
void UNiagaraGPURibbonRendererProperties::InitBindings()
{
...
void UNiagaraGPURibbonRendererProperties::CacheFromCompiledData(const FNiagaraDataSetCompiledData* CompiledData)
{
	...
	// Initialize layout
	RendererLayout.Initialize(ENiagaraGPURibbonVFLayout::Num);
	RendererLayout.SetVariableFromBinding(CompiledData, PositionBinding, ENiagaraGPURibbonVFLayout::Position);
	RendererLayout.SetVariableFromBinding(CompiledData, VelocityBinding, ENiagaraGPURibbonVFLayout::Velocity);
	RendererLayout.SetVariableFromBinding(CompiledData, ColorBinding, ENiagaraGPURibbonVFLayout::Color);
	RendererLayout.SetVariableFromBinding(CompiledData, RibbonWidthBinding, ENiagaraGPURibbonVFLayout::Width);
	RendererLayout.SetVariableFromBinding(CompiledData, RibbonTwistBinding, ENiagaraGPURibbonVFLayout::Twist);
	if (bShouldDoFacing)
	{
		RendererLayout.SetVariableFromBinding(CompiledData, RibbonFacingBinding, ENiagaraGPURibbonVFLayout::Facing);
	}
	RendererLayout.SetVariableFromBinding(CompiledData, NormalizedAgeBinding, ENiagaraGPURibbonVFLayout::NormalizedAge);
	RendererLayout.SetVariableFromBinding(CompiledData, MaterialRandomBinding, ENiagaraGPURibbonVFLayout::MaterialRandom);
	RendererLayout.SetVariableFromBinding(CompiledData, RibbonUVDistance, ENiagaraGPURibbonVFLayout::DistanceFromStart);
	RendererLayout.SetVariableFromBinding(CompiledData, U0OverrideBinding, ENiagaraGPURibbonVFLayout::U0Override);
	RendererLayout.SetVariableFromBinding(CompiledData, V0RangeOverrideBinding, ENiagaraGPURibbonVFLayout::V0RangeOverride);
	RendererLayout.SetVariableFromBinding(CompiledData, U1OverrideBinding, ENiagaraGPURibbonVFLayout::U1Override);
	RendererLayout.SetVariableFromBinding(CompiledData, V1RangeOverrideBinding, ENiagaraGPURibbonVFLayout::V1RangeOverride);
	MaterialParamValidMask  = RendererLayout.SetVariableFromBinding(CompiledData, DynamicMaterialBinding, ENiagaraGPURibbonVFLayout::MaterialParam0) ? 1 : 0;
	MaterialParamValidMask |= RendererLayout.SetVariableFromBinding(CompiledData, DynamicMaterial1Binding, ENiagaraGPURibbonVFLayout::MaterialParam1) ? 2 : 0;
	MaterialParamValidMask |= RendererLayout.SetVariableFromBinding(CompiledData, DynamicMaterial2Binding, ENiagaraGPURibbonVFLayout::MaterialParam2) ? 4 : 0;
	MaterialParamValidMask |= RendererLayout.SetVariableFromBinding(CompiledData, DynamicMaterial3Binding, ENiagaraGPURibbonVFLayout::MaterialParam3) ? 8 : 0;

	RendererLayout.SetVariableFromBinding(CompiledData, RibbonLinkOrderBinding, ENiagaraGPURibbonVFLayout::LinkOrder);
...
bool UNiagaraGPURibbonRendererProperties::IsSupportedVariableForBinding(const FNiagaraVariableBase& InSourceForBinding, const FName& InTargetBindingName) const
{
...
void UNiagaraGPURibbonRendererProperties::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	FName PropertyName = PropertyChangedEvent.GetPropertyName();
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraGPURibbonRendererProperties, TessellationAngle))
	{
...
const TArray<FNiagaraVariable>& UNiagaraGPURibbonRendererProperties::GetOptionalAttributes()
{
...
void UNiagaraGPURibbonRendererProperties::GetRendererWidgets(const FNiagaraEmitterInstance* InEmitter, TArray<TSharedPtr<SWidget>>& OutWidgets, TSharedPtr<FAssetThumbnailPool> InThumbnailPool) const
{
...
void UNiagaraGPURibbonRendererProperties::GetRendererTooltipWidgets(const FNiagaraEmitterInstance* InEmitter, TArray<TSharedPtr<SWidget>>& OutWidgets, TSharedPtr<FAssetThumbnailPool> InThumbnailPool) const
{
	TArray<UMaterialInterface*> Materials;
	GetUsedMaterials(InEmitter, Materials);
	if (Materials.Num() > 0)
...
void UNiagaraGPURibbonRendererProperties::GetRendererFeedback(const UNiagaraEmitter* InEmitter, TArray<FText>& OutErrors, TArray<FText>& OutWarnings, TArray<FText>& OutInfo) const
{
	Super::GetRendererFeedback(InEmitter, OutErrors, OutWarnings, OutInfo);
}


bool UNiagaraGPURibbonRendererProperties::IsMaterialValidForRenderer(UMaterial* InMaterial, FText& InvalidMessage)
{
...
void UNiagaraGPURibbonRendererProperties::FixMaterial(UMaterial* InMaterial)
{
	InMaterial->Modify();
...
bool UNiagaraGPURibbonRendererProperties::CanEditChange(const FProperty* InProperty) const
{

	if (InProperty->HasMetaData(TEXT("Category")) && InProperty->GetMetaData(TEXT("Category")).Contains("Tessellation"))
	{
		FName PropertyName = InProperty->GetFName();
		if (PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraGPURibbonRendererProperties, CurveTension))
		{
			return TessellationMode != ENiagaraGPURibbonTessellationMode::Disabled;
		}
		if (PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraGPURibbonRendererProperties, TessellationFactor))
		{
			return TessellationMode == ENiagaraGPURibbonTessellationMode::Custom;
		}
		if (PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraGPURibbonRendererProperties, TessellationMode))
		{
...
void UNiagaraGPURibbonRendererProperties::RenameVariable(const FNiagaraVariableBase& OldVariable, const FNiagaraVariableBase& NewVariable, const UNiagaraEmitter* InEmitter)
{
...
void UNiagaraGPURibbonRendererProperties::RemoveVariable(const FNiagaraVariableBase& OldVariable, const UNiagaraEmitter* InEmitter)
{
